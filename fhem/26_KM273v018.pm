# $Id: 26_KM273.pm 27126 2023-01-26 18:55:20Z mike3436 $
########################################################################################################################
#
#     26_KM273.pm
...
#   Retrieving the following table could also be done online
#   The table also contains a manually determined 'format' and a 'read' column for defining a cyclic read request
# 
#   R01FD7FE0 0
#   T09FD7FE0 7 0001298A06FD02
#   T01FD3FE0 8 00004E2000000000              << Prepare request, 20000 bytes (0x00004E20) starting at offset 0
#   R01FDBFE0 0                               << Request list
...
#   ...
#   T01FD3FE0 8 00004E2000004E20              << Prepare next request, 20000 bytes starting at offset 20000 (0x00004E20)
#   R01FDBFE0 0                               << Request list
#   ...
#   T01FD3FE0 8 00004E2000009C40              << Prepare next request, 20000 bytes starting at offset 40000 (0x00009C40)
#   ...
#   T01FD3FE0 8 00004E200000EA60              << Prepare next request, 20000 bytes starting at offset 60000 (0x0000EA60)
#   ...
#   T09FDFFE0 4 9434D9B6                      << End of list
#
#   I could not interpret all information from the table,
#   but at least the most interesting ones for me:
#   A constant header of 21 bytes, 1 byte text length followed by text terminated with 0:
#
#   Index ????????????? UpperLimit LowerLimit TextLength Text
    ...
    while (($dir eq 'T') || ($dir eq 'R'))
    {
      ...
      if ($dir eq 'T')
      {
        if (hex $canId == 0x09FDBFE0)
        {
          ...
          {
            #if the readCounter is still not reached after the second read and the data length is identical, set readCounter = readIndex
