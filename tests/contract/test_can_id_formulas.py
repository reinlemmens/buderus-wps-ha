...
class TestCanIdCalculationForRealParameters:
    ...
    def test_idx_0_can_ids_match_expected(self, known_parameters):
        ...
        print(f"✓ idx=0 ({param.text}): read=0x{param.get_read_can_id():08X}, write=0x{param.get_write_can_id():08X}")
    ...
    def test_idx_1_can_ids_match_expected(self, known_parameters):
        ...
        print(f"✓ idx=1 ({param.text}): read=0x{param.get_read_can_id():08X}, write=0x{param.get_write_can_id():08X}")
    ...
    def test_idx_11_can_ids_match_expected(self, known_parameters):
        ...
        print(f"✓ idx=11 ({param.text}): read=0x{param.get_read_can_id():08X}, write=0x{param.get_write_can_id():08X}")
    ...
    def test_formula_consistency_across_all_parameters(self):
        ...
        for param_data in PARAMETER_DATA:
            ...
            if actual_read != expected_read:
                errors.append(f"idx={idx} read: got 0x{actual_read:08X}, expected 0x{expected_read:08X}")
            if actual_write != expected_write:
                errors.append(f"idx={idx} write: got 0x{actual_write:08X}, expected 0x{expected_write:08X}")

        assert not errors, f"CAN ID formula inconsistencies:\n" + "\n".join(errors)
