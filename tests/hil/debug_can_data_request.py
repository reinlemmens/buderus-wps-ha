...
while time.time() - start < 1.0:
    ...
    if frame and frame.arbitration_id == response_id:
        print(f"Response: id=0x{frame.arbitration_id:08X} dlc={frame.dlc} data={frame.data.hex()}")
        break

# Try 2: Data frame with empty data
print("\n=== Method 2: Data Frame (empty) ===")
request = CANMessage(arbitration_id=request_id, data=b'', is_extended_id=True, is_remote_frame=False)
adapter.flush_input_buffer()
slcan = request.to_usbtin_format()
print(f"Sending: {slcan}")
adapter._write_command(slcan.encode('ascii'))
...
request = CANMessage(arbitration_id=request_id, data=b'\x00', is_extended_id=True, is_remote_frame=False)
adapter.flush_input_buffer()
slcan = request.to_usbtin_format()
print(f"Sending: {slcan}")
adapter._write_command(slcan.encode('ascii'))
