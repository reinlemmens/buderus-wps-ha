...
request = CANMessage(
    arbitration_id=count_request_id, data=b"", is_extended_id=True, is_remote_frame=True
)
...
while time.time() - start < 3.0:
    ...
    if frame:
        ...
        print(
            f"  Received: ID=0x{frame.arbitration_id:08X} DLC={frame.dlc} DATA={data_hex}"
        )
...
print("Step 3: Send RTR to 0x01FDBFE0 to retrieve data")
rtr_request_id = 0x01FDBFE0
rtr_response_id = 0x09FDBFE0  # Expected response ID
request = CANMessage(
    arbitration_id=rtr_request_id, data=b"", is_extended_id=True, is_remote_frame=True
)
adapter._write_command(request.to_usbtin_format().encode("ascii"))
