...
#
# Usage: ./scripts/deploy-beta.sh [--no-restart]
#
# This script:
...
# Step 1: Skipped (Library is vendored in source)

# Step 2: Build the zip
echo "[1/3] Building release zip..."
...
# Optional: Restart HA
# Optional: Restart HA (Default: Yes)
if [[ "$1" == "--no-restart" ]]; then
    echo "      Skipping restart (requested via --no-restart)"
    echo ""
