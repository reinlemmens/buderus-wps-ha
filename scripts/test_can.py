...
def test_monitor(duration: float):
    ...
    try:
        ...
        for can_id, f in sorted(by_id.items()):
            ...
            print(f"  ID=0x{can_id:08X} Base=0x{base:04X} Idx={idx:3d} DLC={len(data)} Data={data.hex():16s} {temp_str}")
...
def test_discovery():
    ...
    try:
        ...
        result = can.send_extended_frame(ELEMENT_DATA_REQUEST, request_data, timeout=0.5)

        # Collect responses
        print("   Collecting data responses...")
        frames = can.collect_frames(duration=5.0)

        data_frames = [f for f in frames if f['type'] == 'data' and f['id'] == ELEMENT_DATA_RESPONSE]
        print(f"   Received {len(data_frames)} data frames from 0x{ELEMENT_DATA_RESPONSE:08X}")
        ...
        if data_frames:
            ...
            if len(all_data) >= 18:
                ...
                if name_len > 0 and len(all_data) >= 18 + name_len:
                    name = all_data[18:18+name_len-1].decode('ascii', errors='replace')
                    print(f"     name: {name}")
